...
{
    internal class OfficeInteropBenchmarkRunner : BenchmarkRunner<Excel.Range>
    {
        private readonly Excel.Application _excelApp = new();
        ...
        {
            var workbook = _excelApp.Workbooks.Open(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) + "\\" + _largeFileName);
            if (savingResultingFile)
            {
                workbook.SaveAs(
                    Filename: Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) + "\\" + LoadingLargeFileName,
                    FileFormat: XlFileFormat.xlWorkbookDefault,
                    Password: Type.Missing,
                    WriteResPassword: Type.Missing,
                    ReadOnlyRecommended: false,
